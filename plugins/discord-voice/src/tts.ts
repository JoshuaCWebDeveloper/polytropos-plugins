...
type PollyModule = {
  PollyClient: new (options: Record<string, unknown>) => {
    send(command: unknown): Promise<{ AudioStream?: { transformToByteArray(): Promise<Uint8Array> } }>;
  };
  SynthesizeSpeechCommand: new (input: Record<string, unknown>) => unknown;
};

type EdgeTTSConstructor = new (options: Record<string, unknown>) => {
...
type KokoroModule = {
  KokoroTTS: {
    from_pretrained(modelId: string, options: Record<string, unknown>): Promise<KokoroInstance>;
  };
};
...
let sharedKokoroInstance: KokoroInstance | null = null;
...
export class KokoroTTSProvider implements TTSProvider {
    ...
    if (!sharedInitPromise) {
      sharedInitPromise = (async () => {
        try {
          ...
          const { KokoroTTS } = requireRuntimeModule<KokoroModule>("kokoro-js");
  ...
  async synthesize(text: string): Promise<TTSResult> {
    ...
    const requestedVoice = this.voice;
...
export class PollyTTS implements TTSProvider {
  ...
  private PollyCommand: PollyModule["SynthesizeSpeechCommand"];

  constructor(config: DiscordVoiceConfig) {
    const pollyModule = requireRuntimeModule<PollyModule>("@aws-sdk/client-polly");
    const polly = config.polly ?? {};
    const region = polly.region ?? process.env["AWS_REGION"] ?? "us-east-1";
    this.client = new pollyModule.PollyClient({
      region,
      credentials:
    ...
    this.PollyCommand = pollyModule.SynthesizeSpeechCommand;
    this.voiceId = polly.voiceId ?? "Joanna";
    this.engine = polly.engine;
  }

  async synthesize(text: string): Promise<TTSResult> {
    const command = new this.PollyCommand({
      Text: text,
...
export class EdgeTTSProvider implements TTSProvider {
  ...
  async synthesize(text: string): Promise<TTSResult> {
    ...
    try {
      const EdgeTTS = requireRuntimeModule<EdgeTTSConstructor>("node-edge-tts");
