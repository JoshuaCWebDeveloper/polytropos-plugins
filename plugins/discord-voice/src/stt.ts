...
export interface STTResult {
  ...
  confidence?: number;
  language?: string;
  // Optional segment timing info (for alignment / smart overlap)
  segments?: Array<{ t0: number; t1: number; text: string; probability?: number | null }>;
}
...
function getTransformers(): TransformersModule {
  if (!transformersModule) {
    transformersModule = requireRuntimeModule<TransformersModule>("@xenova/transformers");
...
export class LocalWhisperSTT implements STTProvider {
  ...
  private convertPcmToFloat32(pcmBuffer: Buffer, inputSampleRate: number): Float32Array {
    const inputSamples = new Int16Array(pcmBuffer.buffer, pcmBuffer.byteOffset, Math.floor(pcmBuffer.length / 2));
    if (inputSampleRate === 16000) {
      const out = new Float32Array(inputSamples.length);
      for (let i = 0; i < inputSamples.length; i++) {
        out[i] = inputSamples[i]! / 0x8000;
      }
      return out;
    }

    const outputLength = Math.max(1, Math.round((inputSamples.length * 16000) / inputSampleRate));
    const out = new Float32Array(outputLength);
    const ratio = inputSampleRate / 16000;
    for (let i = 0; i < outputLength; i++) {
      const sourceIndex = i * ratio;
      const lower = Math.floor(sourceIndex);
      const upper = Math.min(lower + 1, inputSamples.length - 1);
      const weight = sourceIndex - lower;
      const sample =
        (inputSamples[lower] ?? 0) * (1 - weight) +
        (inputSamples[upper] ?? inputSamples[lower] ?? 0) * weight;
      out[i] = sample / 0x8000;
    }
    return out;
  }
...
export class WyomingWhisperSTT implements STTProvider {
  ...
  async transcribe(audioBuffer: Buffer, sampleRate: number): Promise<STTResult> {
    return new Promise((resolve, reject) => {
      let resolved = false;
      let lastSegments: STTResult["segments"] | undefined;
      const finish = (text: string) => {
        if (resolved) return;
        resolved = true;
        socket.removeAllListeners();
        socket.end();
        resolve({ text: text.trim(), language: this.language, segments: lastSegments });
      };
      ...
      const socket = net.createConnection({ host: this.host, port: this.port }, () => {
        ...
        const processEvent = (h: { type: string }, data: { text?: string; context?: any }) => {
          // Capture optional segment timing info if present
          if (data?.context?.segments && Array.isArray(data.context.segments)) {
            lastSegments = data.context.segments
              .filter((s: any) => s && typeof s.t0 === "number" && typeof s.t1 === "number")
              .map((s: any) => ({
                t0: s.t0,
                t1: s.t1,
