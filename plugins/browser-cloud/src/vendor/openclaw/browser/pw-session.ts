...
function getHeadersWithAuth(url: string, headers: Record<string, string> = {}) {
...
async function resolvePlaywrightCdpEndpoint(opts: {
  ...
  if (u.protocol === "http:" || u.protocol === "https:") {
    ...
    throw new Error(
      `CDP endpoint not ready (no webSocketDebuggerUrl at ${normalized} within ${opts.timeoutMs}ms)`,
    );
  }
...
export async function createPageViaPlaywright(opts: {
  ...
  try {
    ...
    try {
      const targets = (await session.send("Target.getTargets")) as any;
    ...
    try {
      ...
      try {
        ...
        const ids = Array.isArray(targets?.targetInfos)
          ? targets.targetInfos.map((t: any) => String(t?.targetId ?? "").trim()).filter(Boolean)
          : [];
        // eslint-disable-next-line no-console
