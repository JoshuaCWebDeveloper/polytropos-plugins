...
import * as cdpModule from "./cdp.ts";
import { InvalidBrowserNavigationUrlError } from "./navigation-guard.ts";
import { createBrowserRouteContext } from "./server-context.ts";
import { makeManagedTabsWithNew, makeState, originalFetch, } from "./server-context.remote-tab-ops.harness.ts";
afterEach(() => {
    globalThis.fetch = originalFetch;
    vi.restoreAllMocks();
});
function seedRunningProfileState(state, profileName = "openclaw") {
...
async function expectOldManagedTabClose(fetchMock) {
    await vi.waitFor(() => {
        expect(fetchMock).toHaveBeenCalledWith(expect.stringContaining("/json/close/OLD1"), expect.any(Object));
    });
}
function createOldTabCleanupFetchMock(existingTabs, params) {
    return vi.fn(async (url) => {
        const value = String(url);
        if (value.includes("/json/list")) {
            return { ok: true, json: async () => existingTabs };
        }
...
function createManagedTabListFetchMock(params) {
    return vi.fn(async (url) => {
        const value = String(url);
        if (value.includes("/json/list")) {
            return { ok: true, json: async () => params.existingTabs };
        }
...
describe("browser server-context tab selection state", () => {
    ...
    it("never closes the just-opened managed tab during cap cleanup", async () => {
        ...
        expect(opened.targetId).toBe("NEW");
        await expectOldManagedTabClose(fetchMock);
        expect(fetchMock).not.toHaveBeenCalledWith(expect.stringContaining("/json/close/NEW"), expect.anything());
    });
    ...
    it("does not run managed tab cleanup in attachOnly mode", async () => {
        ...
        const opened = await openclaw.openTab("http://127.0.0.1:3009");
        expect(opened.targetId).toBe("NEW");
        expect(fetchMock).not.toHaveBeenCalledWith(expect.stringContaining("/json/close/"), expect.anything());
    });
    it("does not block openTab on slow best-effort cleanup closes", async () => {
        ...
        const opened = await Promise.race([
            ...
            new Promise((_, reject) => setTimeout(() => reject(new Error("openTab timed out waiting for cleanup")), 300)),
        ]);
        expect(opened.targetId).toBe("NEW");
    });
    it("blocks unsupported non-network URLs before any HTTP tab-open fallback", async () => {
        ...
        const ctx = createBrowserRouteContext({ getState: () => state });
        const openclaw = ctx.forProfile("openclaw");
        await expect(openclaw.openTab("file:///etc/passwd")).rejects.toBeInstanceOf(InvalidBrowserNavigationUrlError);
        expect(fetchMock).not.toHaveBeenCalled();
    });
});
//# sourceMappingURL=server-context.tab-selection-state.suite.js.map