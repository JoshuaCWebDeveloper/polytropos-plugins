...
import { deriveDefaultBrowserCdpPortRange, deriveDefaultBrowserControlPort, DEFAULT_BROWSER_CONTROL_PORT, } from "../config/port-defaults.ts";
import { isLoopbackHost } from "../gateway/net.ts";
import { DEFAULT_OPENCLAW_BROWSER_COLOR, DEFAULT_OPENCLAW_BROWSER_ENABLED, DEFAULT_BROWSER_EVALUATE_ENABLED, DEFAULT_BROWSER_DEFAULT_PROFILE_NAME, DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, } from "./constants.ts";
import { CDP_PORT_RANGE_START, getUsedPorts } from "./profiles.ts";
const BROWSER_USE_CLOUD_CDP_PLACEHOLDERS = new Set([
    "browser-use",
    "browser-use-cloud",
    "browser-use:cloud",
]);
export function isBrowserUseCloudPlaceholderCdpUrl(raw) {
    const trimmed = raw.trim();
    return trimmed.length > 0 && BROWSER_USE_CLOUD_CDP_PLACEHOLDERS.has(trimmed);
}
...
function resolveCdpPortRangeStart(rawStart, fallbackStart, rangeSpan) {
    ...
    if (start > maxStart) {
        throw new Error(`browser.cdpPortRangeStart (${start}) is too high for a ${rangeSpan + 1}-port range; max is ${maxStart}.`);
    }
    return start;
}
...
function resolveBrowserSsrFPolicy(cfg) {
    ...
    if (!resolvedAllowPrivateNetwork &&
        !hasExplicitPrivateSetting &&
        !allowedHostnames &&
        !hostnameAllowlist) {
        return undefined;
...
export function resolveProfile(resolved, profileName) {
    ...
    const hasExplicitCdpUrl = profile.cdpUrl !== undefined;
    ...
    const isPlaceholderUrl = isBrowserUseCloudPlaceholderCdpUrl(rawProfileUrl);
    const shouldSkipDerivingCdpUrl = driver === "clawd" && attachOnly && (isPlaceholderUrl || (hasExplicitCdpUrl && !rawProfileUrl));
    if (rawProfileUrl && !isPlaceholderUrl) {
        const parsed = parseHttpUrl(rawProfileUrl, `browser.profiles.${profileName}.cdpUrl`);
        cdpHost = parsed.parsed.hostname;
        cdpPort = parsed.port;
        cdpUrl = parsed.normalized;
    }
    else if (shouldSkipDerivingCdpUrl) {
        // For clawd attachOnly profiles, allow a blank or placeholder cdpUrl so the
        // browser control service can provision an attach target (e.g. Browser Use Cloud).
        cdpUrl = rawProfileUrl;
    }
    else if (cdpPort) {
