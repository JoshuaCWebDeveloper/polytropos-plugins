...
import { CHROME_BOOTSTRAP_EXIT_TIMEOUT_MS, CHROME_BOOTSTRAP_PREFS_TIMEOUT_MS, CHROME_LAUNCH_READY_POLL_MS, CHROME_LAUNCH_READY_WINDOW_MS, CHROME_REACHABILITY_TIMEOUT_MS, CHROME_STDERR_HINT_MAX_CHARS, CHROME_STOP_PROBE_TIMEOUT_MS, CHROME_STOP_TIMEOUT_MS, CHROME_WS_READY_TIMEOUT_MS, } from "./cdp-timeouts.ts";
import { appendCdpPath, fetchCdpChecked, openCdpWebSocket } from "./cdp.helpers.ts";
import { normalizeCdpWsUrl } from "./cdp.ts";
import { resolveBrowserExecutableForPlatform, } from "./chrome.executables.ts";
import { decorateOpenClawProfile, ensureProfileCleanExit, isProfileDecorated, } from "./chrome.profile-decoration.ts";
import { DEFAULT_OPENCLAW_BROWSER_COLOR, DEFAULT_OPENCLAW_BROWSER_PROFILE_NAME, } from "./constants.ts";
...
export async function isChromeReachable(cdpUrl, timeoutMs = CHROME_REACHABILITY_TIMEOUT_MS) {
    // Some providers (e.g. Browser Use Cloud) return a ws:// CDP endpoint directly.
    // Treat a healthy websocket as "reachable".
    try {
...
export async function getChromeWebSocketUrl(cdpUrl, timeoutMs = CHROME_REACHABILITY_TIMEOUT_MS) {
    // Some providers already provide a websocket endpoint.
    try {
        const u = new URL(cdpUrl);
        if (u.protocol === "ws:" || u.protocol === "wss:") {
            return cdpUrl;
