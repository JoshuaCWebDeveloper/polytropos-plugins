import { DEFAULT_FILL_FIELD_TYPE } from "./form-fields.ts";
import { DEFAULT_UPLOAD_DIR, resolveStrictExistingPathsWithinRoot } from "./paths.ts";
import { connectBrowser, ensurePageState, findPageByTargetId, forceDisconnectPlaywrightForTarget, getPageForTargetId, refLocator, restoreRoleRefsForTarget, } from "./pw-session.ts";
import { normalizeTimeoutMs, requireRef, toAIFriendlyError } from "./pw-tools-core.shared.ts";
const MAX_UPLOAD_RETRIES = 3;
const UPLOAD_BACKOFF_MS = 1000;
export async function uploadFilesToPage(opts) {
    ...
    for (let attempt = 0; attempt < MAX_UPLOAD_RETRIES; attempt++) {
        try {
            ...
            // If the error is about CDP endpoint not ready, it means the BU session is still starting up.
            // We should retry in this case as well.
            if (err instanceof Error && err.message.includes("CDP endpoint not ready") && attempt < MAX_UPLOAD_RETRIES - 1) {
                // Force disconnect and reconnect to get a fresh session
                await forceDisconnectPlaywrightForTarget({ cdpUrl: opts.cdpUrl, targetId: opts.targetId });
                // Also try to get a fresh session from Browser Use Cloud
                await forceDisconnectPlaywrightForTarget({ cdpUrl: opts.cdpUrl });
                await new Promise(r => setTimeout(r, UPLOAD_BACKOFF_MS * (attempt + 1)));
                continue;
            }
