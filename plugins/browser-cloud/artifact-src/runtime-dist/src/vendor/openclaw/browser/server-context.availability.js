import { PROFILE_ATTACH_RETRY_TIMEOUT_MS, PROFILE_POST_RESTART_WS_TIMEOUT_MS, resolveCdpReachabilityTimeouts, } from "./cdp-timeouts.ts";
import { isChromeCdpReady, isChromeReachable, launchOpenClawChrome, stopOpenClawChrome, } from "./chrome.ts";
import { ensureChromeExtensionRelayServer, stopChromeExtensionRelayServer, } from "./extension-relay.ts";
import { CDP_READY_AFTER_LAUNCH_MAX_TIMEOUT_MS, CDP_READY_AFTER_LAUNCH_MIN_TIMEOUT_MS, CDP_READY_AFTER_LAUNCH_POLL_MS, CDP_READY_AFTER_LAUNCH_WINDOW_MS, } from "./server-context.constants.ts";
export function createProfileAvailability({ opts, profile, state, getProfileState, setProfileRunning, }) {
    ...
    const waitForCdpReadyAfterLaunch = async () => {
        ...
        while (Date.now() < deadlineMs) {
            ...
            const attemptTimeoutMs = Math.max(CDP_READY_AFTER_LAUNCH_MIN_TIMEOUT_MS, Math.min(CDP_READY_AFTER_LAUNCH_MAX_TIMEOUT_MS, remainingMs));
    ...
    const stopRunningBrowser = async () => {
        ...
        const profileState = getProfileState();
        // If this profile is attached to an ephemeral remote target (e.g. Browser Use Cloud),
        // allow callers to stop it even when there is no local running process.
        if (profileState.attachTarget && opts.onStopAttachTarget) {
            const result = await opts.onStopAttachTarget(profile);
            if (result.stopped) {
                profileState.attachTarget = null;
                // Note: we intentionally do not mutate profile.cdpUrl here; callers can
                // re-provision on next ensureBrowserAvailable.
            }
            return result;
        }
        if (!profileState.running) {
...
//# sourceMappingURL=server-context.availability.js.map