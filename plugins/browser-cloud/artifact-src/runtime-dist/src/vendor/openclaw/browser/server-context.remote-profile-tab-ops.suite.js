...
import * as chromeModule from "./chrome.ts";
import * as pwAiModule from "./pw-ai-module.ts";
import { createBrowserRouteContext } from "./server-context.ts";
import { createJsonListFetchMock, createRemoteRouteHarness, createSequentialPageLister, makeState, originalFetch, } from "./server-context.remote-tab-ops.harness.ts";
afterEach(() => {
...
describe("browser server-context remote profile tab operations", () => {
    it("uses profile-level attachOnly when global attachOnly is false", async () => {
        ...
        await expect(ctx.forProfile("openclaw").ensureBrowserAvailable()).rejects.toThrow(/attachOnly is enabled/i);
    ...
    it("keeps attachOnly websocket failures off the loopback ownership error path", async () => {
        ...
        const launchMock = vi.mocked(chromeModule.launchOpenClawChrome);
        const ctx = createBrowserRouteContext({ getState: () => state });
        await expect(ctx.forProfile("openclaw").ensureBrowserAvailable()).rejects.toThrow(/attachOnly is enabled and CDP websocket/i);
    ...
    it("falls back to /json/list when Playwright is not available", async () => {
        ...
        const { remote } = createRemoteRouteHarness(vi.fn(createJsonListFetchMock([
            {
                id: "T1",
                title: "Tab 1",
...
//# sourceMappingURL=server-context.remote-profile-tab-ops.suite.js.map