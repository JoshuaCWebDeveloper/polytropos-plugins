import { CDP_JSON_NEW_TIMEOUT_MS } from "./cdp-timeouts.ts";
import { fetchJson, fetchOk } from "./cdp.helpers.ts";
import { appendCdpPath, createTargetViaCdp, normalizeCdpWsUrl } from "./cdp.ts";
...
import { MANAGED_BROWSER_PAGE_TAB_LIMIT, OPEN_TAB_DISCOVERY_POLL_MS, OPEN_TAB_DISCOVERY_WINDOW_MS, } from "./server-context.constants.ts";
/**
...
export function createProfileTabOps({ profile, state, getProfileState, }) {
    ...
    const openTab = async (url) => {
        ...
        const created = await fetchJson(endpoint, CDP_JSON_NEW_TIMEOUT_MS, {
            method: "PUT",
        }).catch(async (err) => {
            if (String(err).includes("HTTP 405")) {
                return await fetchJson(endpoint, CDP_JSON_NEW_TIMEOUT_MS);
            }
            throw err;
        });
        if (!created.id) {
...
//# sourceMappingURL=server-context.tab-ops.js.map