import { resolveExecSecretRef } from "./openclaw-secrets-lite.ts";
...
function isGatewayRuntime() {
    return (process.env.OPENCLAW_SERVICE_KIND === "gateway" ||
...
`,
        ...
        async execute(_toolCallId, args) {
            ...
            ctrl["sessions"]?.beginAction?.(action);
            try {
                switch (action) {
                    case "status":
                ...
                try {
                    ...
                    ctx = {
                        ...
                        cdpUrl: st?.cdpUrl ?? null,
                        buStatus: st?.diag?.bu?.ok === true ? (st.diag.bu.session?.status ?? null) : null,
                        buFinishedAt: st?.diag?.bu?.ok === true ? (st.diag.bu.session?.finishedAt ?? null) : null,
                    };
                }
                catch {
                    // ignore
                ...
                if (isBrowserCloudFatalError(normalizedErr)) {
                    ctrl["sessions"]?.invalidate?.(`fatal:${action}`);
                }
                // eslint-disable-next-line no-console
